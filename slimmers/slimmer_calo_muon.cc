...
void slimmer_calo_muon(TString fname)
{
  ...
  TString foutname = dir + fname + "/out_muon_skimmed.root";
  ...
  oldtree->SetBranchAddress("trk_mcs_muon_mom", &trk_mcs_muon_mom);
  oldtree->SetBranchAddress("trk_range_muon_mom", &trk_range_muon_mom);
  ...
  for (auto i : ROOT::TSeqI(nentries))
  {
    ...
    if (
         ...
         (trk_distance > 0) &&
         (trk_distance < 5) &&
         (abs((trk_mcs_muon_mom-trk_range_muon_mom)/trk_range_muon_mom) < 0.5)
       )
    {
      newtree->Fill();
    }// if cuts pass
